...
"use client";
import { BookOpen, MessageCircle, Send, ThumbsUp } from "lucide-react";
import { useState } from "react";

interface StoryViewCardProps {
  data: {
...
export function StoryViewCard({ data }: StoryViewCardProps) {
  ...
  const handleAddComment = () => {
    if (commentInput.trim()) {
      const newComment = {
        id: Math.max(0, ...commentsList.map((c: any) => c.id)) + 1,
          ...
          return (
            <div key={replyPath}>
                ...
                <div className="flex gap-2 mt-2 ml-8">
                  ...
                  <input
                    ...
                    onKeyPress={(e: React.KeyboardEvent<HTMLInputElement>) => {
                      if (e.key === "Enter") handleAddReply(parentId, nestedReply.id);
                    }}
              ...
              {nestedReply.replies && nestedReply.replies.length > 0 &&
                renderNestedReplies(nestedReply.replies, nestedReply.id, nestedReply.id, newAncestry)
              }
            </div>
          );
        })}
                ...
                <div key={comment.id} className="flex gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    {comment.replies && comment.replies.length > 0 &&
                      renderNestedReplies(comment.replies, comment.id, undefined, [comment.id])}
                  </div>
                </div>
              ))}
            </div>
