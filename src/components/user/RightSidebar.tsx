...
function UserCard({ person, onProfileClick, onFollow }: any) {
  return (
    <div className="flex items-center gap-3">
      <img
        src={person.profile_photo || 'https://ui-avatars.com/api/?name=User'}
        alt={person.username}
        className="w-10 h-10 rounded-full object-cover cursor-pointer"
        onClick={() => onProfileClick(person)}
      />
      <div className="flex-1 cursor-pointer" onClick={() => onProfileClick(person)}>
        <h3 className="text-sm font-medium">
          {person.full_name || person.username}
        </h3>
        <p className="text-xs text-gray-500">
          {person.profession || 'User'}
        </p>
      </div>
      <button
...
export function RightSidebar() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [peopleRes, groupsRes, storiesRes, hotRes] =
          await Promise.all([
        ...
        const questionPosts = hotPosts
          .filter(
            (p: any) =>
              (p.post_type || p.type)?.toUpperCase() === 'QUESTION'
          )
  ...
  const handleTrendingClick =
    (type: 'question' | 'story') => (item: any) => {
      open(type, item)
    }

  return (
    <aside className="w-80 bg-white rounded-2xl shadow-sm p-6 space-y-6">

      {/* 🔥 TRENDING QUESTIONS */}
      <div>
        <div className="flex items-center gap-2 mb-4">
          <TrendingUp className="w-5 h-5" />
          <h2 className="font-semibold">Trending Questions</h2>
        </div>

        {loading ? (
          <p className="text-xs text-gray-400">Loading...</p>
        ) : questions.length === 0 ? (
          <p className="text-xs text-gray-400">No trending questions</p>
        ) : (
          questions.map((q: any) => (
            <div
              key={q.id}
              className="text-sm cursor-pointer mb-3 hover:underline"
              onClick={() => handleTrendingClick('question')(q)}
            >
              {q.title || q.content}
            </div>
          ))
      ...
      {/* 📖 STORIES */}
      <div>
        <h2 className="font-semibold mb-4">Latest Stories</h2>
        ...
        {loading ? (
          ...
          stories.map((story: any) => (
            <div
              key={story.id}
              className="text-sm cursor-pointer mb-3 hover:underline"
              onClick={() => handleTrendingClick('story')(story)}
            >
              {story.title}
            </div>
          ))
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {groups.slice(0, 5).map((group) => (
              <div key={group.id} className="p-3 border rounded-xl">
